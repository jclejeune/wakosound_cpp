...
QPushButton#fxBtn:checked {
    ...
    void MixerPanel::buildStrip(int pad, QWidget *container)
    {
        ...
        if (!fxWindows_[pad]) {
            ...
            // ── Sync bouton ← croix de la fenêtre ────────────────
    ...
    {
        // ── Reset visuel : tous les sliders à 100 ────────────────────
        for (auto *child : findChildren<QSlider *>())
            child->setValue(100);

        // ── Reset audio : volumes tracks ─────────────────────────────
        for (int i = 0; i < seq::MAX_PADS; ++i)
            pattern_->setTrackVolume(i, 1.0f);

        // ── Reset audio : master volume ───────────────────────────────
        ...
        // ── Fermer les fenêtres FX + décocher les boutons ─────────────────
        for (int i = 0; i < seq::MAX_PADS; ++i)
        {
            if (fxWindows_[i])
                fxWindows_[i]->hide();
            if (fxButtons_[i])
                fxButtons_[i]->setChecked(false);
        }
        // Master
        constexpr int MASTER_IDX = seq::MAX_PADS;
        if (fxWindows_[MASTER_IDX])
