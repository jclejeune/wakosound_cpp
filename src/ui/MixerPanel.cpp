...
QPushButton#muteBtn:checked {
    background:#CC4444; color:#FFF; border-color:#FF6666;
}
QPushButton#soloBtn {
...
QPushButton#soloBtn:checked {
    background:#BB9900; color:#1A1A00; border-color:#FFCC00;
}
QPushButton#fxBtn {
...
QPushButton#fxBtn:checked {
    background:#2A5A8A; color:#AADDFF; border-color:#4A9ADF;
}
)";
...
void MixerPanel::buildStrip(int pad, QWidget* container) {
    ...
    // Slider volume — stocké dans trackSliders_
    auto* slider = new QSlider(Qt::Vertical);
    slider->setRange(0, 100);
    slider->setValue(100);
    slider->setFixedHeight(70);
    trackSliders_[pad] = slider;   // ← stockage explicite
    ...
    connect(slider, &QSlider::valueChanged, this, [dbLbl](int v) {
        dbLbl->setText(v == 0 ? "-∞"
            : QString::number(int(20.f * std::log10(v / 100.f))) + "dB");
    });
    lay->addWidget(dbLbl);
    ...
    connect(fxBtn, &QPushButton::clicked, this, [this, pad, fxBtn](bool checked) {
        if (!fxWindows_[pad]) {
            ...
            fxWindows_[pad] = win;
            connect(win, &EffectWindow::closed, fxBtn, [fxBtn]() {
...
void MixerPanel::buildMasterStrip(QWidget* container) {
    ...
    // Slider master — stocké dans masterSlider_
    auto* slider = new QSlider(Qt::Vertical);
    slider->setRange(0, 100);
    slider->setValue(100);
    slider->setFixedHeight(70);
    masterSlider_ = slider;   // ← stockage explicite
    ...
    connect(masterFxBtn_, &QPushButton::clicked, this, [this](bool checked) {
        ...
        if (!fxWindows_[MASTER_IDX]) {
            ...
            connect(win, &EffectWindow::closed, masterFxBtn_, [this]() {
                masterFxBtn_->setChecked(false);
            });
        }
        if (checked) {
            fxWindows_[MASTER_IDX]->show();
            fxWindows_[MASTER_IDX]->raise();
        } else {
            fxWindows_[MASTER_IDX]->hide();
        }
    });
...
// ──────────────────────────────────────────────────────────────────
// resetAll — reset complet visuel + audio + effets
// ──────────────────────────────────────────────────────────────────
void MixerPanel::resetAll() {
    // ── 1. Détruire les fenêtres FX ──────────────────────────────
    // On détruit et recrée à la demande — garantit que les widgets
    // reflètent l'état réel des chaînes d'effets à la réouverture.
    for (int i = 0; i < seq::MAX_PADS + 1; ++i) {
        if (fxWindows_[i]) {
            delete fxWindows_[i];
            fxWindows_[i] = nullptr;
        }
    }
    for (int i = 0; i < seq::MAX_PADS; ++i)
        if (fxButtons_[i]) fxButtons_[i]->setChecked(false);
    if (masterFxBtn_) masterFxBtn_->setChecked(false);

    // ── 2. Reset sliders volume (stockés explicitement) ───────────
    for (int i = 0; i < seq::MAX_PADS; ++i)
        if (trackSliders_[i]) trackSliders_[i]->setValue(100);
    if (masterSlider_) masterSlider_->setValue(100);

    // ── 3. Reset volumes + mute/solo dans le pattern ──────────────
    for (int i = 0; i < seq::MAX_PADS; ++i)
        pattern_->setTrackVolume(i, 1.0f);
    ...
    // ── 4. Reset effect chains (9 tracks + master) ────────────────
    auto resetChain = [](audio::EffectChain& chain) {
        ...
        chain.sat().setMode(audio::Saturator::Mode::Transistor);
        chain.sat().setDrive(0.f);
        chain.sat().setMix(1.f);

        for (int b = 0; b < 5; ++b)
            chain.eq().setBandGain(b, 0.f);
        ...
        // Vider les buffers internes (stoppe le feedback résiduel)
        chain.reset();
    };
